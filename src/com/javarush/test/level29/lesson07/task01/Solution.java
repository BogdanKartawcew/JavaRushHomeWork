        ...
        new Thread() {
            ...
            public void run() {
                for (int i = 0; i < 100000; i++) {
                    messageQueue.add(String.valueOf(i/*--*/)); // тут не треба видаляти комент
                }
            }
        }.start();
    }
        ...
        for (int i = 0; i < NUMBER_OF_THREADS; i++) {
            new Thread() {
                ...
                public void run() {
                    while (true) {
                        try {
                            messageQueue.drainTo(/*messageQueue*/ batch, MAX_BATCH_SIZE); //закоментоване треба видалити, щоб пройшло
                            persistData(batch);
                            batch.clear();
                            Thread.sleep(1);
                        } catch (Throwable e) {
