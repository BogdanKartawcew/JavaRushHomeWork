...
/*вспомогательный класс для инициализации и настройки меню*/
{
    ...
    {
        JMenuItem newMenu = new JMenuItem(text); //Создавать новый пункт меню JMenuItem, используя text
    ...
    {
     /*Подсказка: при создании JMenuItem передай в конструктор action. Разберись откуда возьмется имя пункта меню,
     если его не устанавливать дополнительно.*/

        JMenuItem newMenu = new JMenuItem(action);
        //newMenu.setText((String) action.getValue(action.NAME));
        parent.add(newMenu);//Добавлять в parent созданный пункт меню.
        return newMenu; //Возвращать созданный пункт меню
    }

    public static JMenuItem addMenuItem(JMenu parent, String text, Action action) //добавляємо нове меню
    /*который добавляет в parent новый пункт меню с текстом text и действием action при выборе этого метода.
    При реализации можешь использовать вызов метода из предыдущего пункта.*/
    {
    ...
    public static void initFontMenu(View view, JMenuBar menuBar)
    {
        ...
        JMenu fontTypeMenu = new JMenu("Стиль шрифту");
