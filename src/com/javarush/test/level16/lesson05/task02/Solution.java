...
public class Solution {
    ...
    public static void main(String[] args) throws InterruptedException {
        List<Horse> horses2 = prepareHorsesAndStart();
        while (calculateHorsesFinished(horses2) != countHorses) {
        }
    }

    public static int calculateHorsesFinished(List<Horse> horses3) throws InterruptedException {
        int countFinished = 0;
        for(int i = 0; i < horses3.size(); i++ ) {
            if(horses3.get(i).isFinished())
            {
                countFinished++ ;
            }
            if(!horses3.get(i).isFinished())
            {
                System.out.println("Waiting for " + horses3.get(i).getName());
                horses3.get(i).join();
                countFinished++;
            }
        }
    ...
    public static List<Horse> prepareHorsesAndStart() {
        List<Horse> horses1 = new ArrayList<Horse>(countHorses);
        String number;
        for (int i = 1; i < countHorses + 1; i++) {
            number = i < 10 ? ("0" + i) : "" + i;
            horses1.add(new Horse("Horse_" + number));
        }
